...
class Tokenizer(object):
    ...
    def extract_term(self, term, filepath, re_pat):
        if (len(term) >= self._term_min_len and len(term) <= self._term_max_len) and (not self.is_stopword(term)):
            if (term in self._terms):
                ...
                self._terms[term] = {'all': 1, 'df': 1, 'type':re_pat, filepath:1}
            return True
        else:
            return False
    ...
    def match_list_re(self, patterns, str, progessbar_str=""):
        ...
        for pattern in self.progressbar(patterns, prefix=progessbar_str) :
            ...
            pat_re += [pattern] * len(tokens_parciales)
            for token in tokens_parciales:
                str = str.replace(token, '#')
            tokens += tokens_parciales
        return pat_re, tokens, str
    ...
    def remove_ambiguious_tokens(self, raw):
        ...
        re_pat, tokens, raw = self.match_list_re(self._pre_re_patterns, raw)
        sentences = split('(?:\.|\?|\!)\s*', raw)
        sentences = split('\n', raw) if len(sentences) == 1 else [lowercase_firstletter(s) for s in sentences if len(s) > 0]
        return re_pat, [token.lower() for token in tokens], sentences


    def tokenize_sentence(self, sentence):
        re_pat, tokens, _ = self.match_list_re(self._post_re_patterns, sentence)
        return re_pat, [token.lower() for token in tokens]
    ...
    def tokenize_file(self, filepath):
        ...
        if (not self.is_binary(filepath)):
            p = open(filepath, "r")
            re_pat, tokens, sentences = self.remove_ambiguious_tokens(p.read())
            ...
            for sentence in self.progressbar( sentences, f" [+] {relpath(filepath)}: "):
                re_pat_tmp, token_tmp = self.tokenize_sentence(sentence)
                tokens += token_tmp
                re_pat += re_pat_tmp
                terms = len([i for i in range(len(tokens)) if (self.extract_term(tokens[i], filepath, re_pat[i]))])
